                    ...
                    <div class="p-6">
                        ...
                        <div class="relative">
                            ...
                            <div class="hidden md:flex absolute top-1/2 left-0 w-full h-1 transform -translate-y-1/2 z-0">
                                ...
                                <div class="h-full flex-1 {{ $hasPickedByCourier ? 'bg-primary-600' : 'bg-gray-200' }}"></div>

                                <!-- Line 2: Picked to On The Way -->
                                <div class="h-full flex-1 {{ $hasOnTheWay ? 'bg-primary-600' : 'bg-gray-200' }}"></div>

                                <!-- Line 3: On The Way to Custom Hold -->
                                <div class="h-full flex-1 {{ $hasCustomHold ? 'bg-amber-500' : 'bg-gray-200' }}"></div>

                                <!-- Line 4: Custom Hold to Delivered -->
                                <div class="h-full flex-1 {{ $hasDelivered ? 'bg-green-500' : 'bg-gray-200' }}"></div>
                            </div>

                            <!-- Mobile Progress Line with Colored Segments -->
                            <div class="md:hidden absolute top-0 left-8 h-full z-0 flex flex-col">
                                <!-- Line 1: To Picked -->
                                <div class="w-1 flex-1 {{ $hasPickedByCourier ? 'bg-primary-600' : 'bg-gray-200' }}"></div>

                                <!-- Line 2: To On The Way -->
                                <div class="w-1 flex-1 {{ $hasOnTheWay ? 'bg-primary-600' : 'bg-gray-200' }}"></div>

                                <!-- Line 3: To Custom Hold -->
                                <div class="w-1 flex-1 {{ $hasCustomHold ? 'bg-amber-500' : 'bg-gray-200' }}"></div>

                                <!-- Line 4: To Delivered -->
                                <div class="w-1 flex-1 {{ $hasDelivered ? 'bg-green-500' : 'bg-gray-200' }}"></div>
                            </div>
                            ...
                            <div class="flex flex-col md:flex-row justify-between relative z-10">
                                ...
                                <div class="flex md:block md:text-center mb-8 md:mb-0">
                                    ...
                                    <div class="ml-4 md:ml-0 md:mt-3">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::parse($courier->date_shipped)->format('M d, Y') }}</p>
                                    </div>
                                </div>

                                <!-- Step 2: Picked by Courier -->
                                <div class="flex md:block md:text-center mb-8 md:mb-0">
                                    <div class="flex-shrink-0 flex items-center justify-center w-16 h-16 md:mx-auto rounded-full {{ $hasPickedByCourier ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg border-4 border-white">
                                    ...
                                    <div class="ml-4 md:ml-0 md:mt-3">
                                        <h4 class="text-sm font-semibold {{ $hasPickedByCourier ? 'text-gray-900' : 'text-gray-500' }}">Picked by Courier</h4>
                                        @if($hasPickedByCourier)
                                            @php
                                                $pickedTrack = $tracks->firstWhere('status', 'Picked by Courier');
                                            @endphp
                                            <p class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::parse($pickedTrack->created_at)->format('M d, Y') }}</p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <!-- Step 3: On The Way -->
                                <div class="flex md:block md:text-center mb-8 md:mb-0">
                                    <div class="flex-shrink-0 flex items-center justify-center w-16 h-16 md:mx-auto rounded-full {{ $hasOnTheWay ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg border-4 border-white">
                                    ...
                                    <div class="ml-4 md:ml-0 md:mt-3">
                                        <h4 class="text-sm font-semibold {{ $hasOnTheWay ? 'text-gray-900' : 'text-gray-500' }}">On The Way</h4>
                                        @if($hasOnTheWay)
                                            @php
                                                $onTheWayTrack = $tracks->firstWhere('status', 'On The Way');
                                            @endphp
                                            <p class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::parse($onTheWayTrack->created_at)->format('M d, Y') }}</p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <!-- Step 4: Custom Hold -->
                                <div class="flex md:block md:text-center mb-8 md:mb-0">
                                    <div class="flex-shrink-0 flex items-center justify-center w-16 h-16 md:mx-auto rounded-full {{ $hasCustomHold ? 'bg-amber-500 text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg border-4 border-white">
                                    ...
                                    <div class="ml-4 md:ml-0 md:mt-3">
                                        <h4 class="text-sm font-semibold {{ $hasCustomHold ? 'text-gray-900' : 'text-gray-500' }}">Custom Hold</h4>
                                        @if($hasCustomHold)
                                            @php
                                                $customHoldTrack = $tracks->firstWhere('status', 'Custom Hold');
                                            @endphp
                                            <p class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::parse($customHoldTrack->created_at)->format('M d, Y') }}</p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="flex md:block md:text-center">
                                    <div class="flex-shrink-0 flex items-center justify-center w-16 h-16 md:mx-auto rounded-full {{ $hasDelivered ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg border-4 border-white">
                                    ...
                                    <div class="ml-4 md:ml-0 md:mt-3">
                                        <h4 class="text-sm font-semibold {{ $hasDelivered ? 'text-gray-900' : 'text-gray-500' }}">Delivered</h4>
                                        @if($hasDelivered)
                                            @php
                                                $deliveredTrack = $tracks->firstWhere('status', 'Delivered');
                                            @endphp
                                            <p class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::parse($deliveredTrack->created_at)->format('M d, Y') }}</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="space-y-6">
                                        @foreach($tracks as $track)
                                            <div class="relative z-10 flex items-start">
                                                <div class="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full {{ $track->status === 'Delivered' ? 'bg-green-500' : ($track->status === 'Custom Hold' ? 'bg-amber-500' : 'bg-primary-600') }} text-white shadow-md border-2 border-white">
                                                ...
                                                <div class="ml-4">
                                                    ...
                                                    <div class="mt-1">
                                                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $track->status === 'Delivered' ? 'bg-green-100 text-green-800' : ($track->status === 'Custom Hold' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800') }}">
                                                            {{ $track->status }}
                                                        </span>
                                                    </div>
                        ...
                        <div class="flex flex-col md:flex-row">
                            ...
                            <div class="md:w-{{ $courier->photo ? '3/4' : 'full' }}">
                                ...
                                <div class="mt-6 flex flex-wrap gap-3 justify-center md:justify-start">
                                    ...
                                    @if($courier->status === 'Custom Hold' && ($courier->clearance_cost > 0 || $courier->cost > 0))
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const originAddress = @json($courier->saddress ?? '');
        const currentAddress = @json($courier->location ?? '');
        const destinationAddress = @json($courier->address ?? '');
        ...
        function initShipmentMap() {
            ...
            if (mapElement.offsetWidth === 0 || mapElement.offsetHeight === 0) {
                // Container still hidden — try again shortly
                setTimeout(initShipmentMap, 200);
                return;
            }
            ...
            // Make sure tiles paint correctly now that the container is visible
            setTimeout(() => map.invalidateSize(), 100);

            const distanceEl = document.getElementById("route_distance");

            function geocode(address) {
                if (!address) return Promise.resolve(null);
                const url = `https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(address)}&accept-language=en`;
            ...
            const stops = [
                { label: 'Origin',          address: originAddress,      color: '#3B82F6' },
                { label: 'Current Location', address: currentAddress,    color: '#FFD700' },
                { label: 'Destination',     address: destinationAddress, color: '#10B981' }
            ];

            Promise.all(stops.map(s => geocode(s.address))).then(coords => {
                const points = [];
                coords.forEach((latlng, i) => {
                    if (!latlng) return;
                    const stop = stops[i];
                    const icon = L.divIcon({
                        html: `<div style="background-color: ${stop.color}; width: 24px; height: 24px; border-radius: 50%; border: 2px solid white; box-shadow: 0 1px 4px rgba(0,0,0,0.4);"></div>`,
                        className: 'custom-div-icon',
                        iconSize: [24, 24],
                        iconAnchor: [12, 12]
                    });
                    const marker = L.marker(latlng, { icon: icon, title: stop.label }).addTo(map);
                    marker.bindPopup(`<b>${stop.label}:</b><br>${stop.address}`);
                    points.push(latlng);
                });

                if (points.length === 0) {
                    if (distanceEl) distanceEl.textContent = "Location not found";
                    return;
                }

                if (points.length >= 2) {
                    L.polyline(points, { color: '#FFD700', weight: 5, opacity: 0.8 }).addTo(map);
                    const bounds = L.latLngBounds(points);
                    map.fitBounds(bounds, { padding: [50, 50], animate: false, maxZoom: 10 });
                    if (distanceEl) distanceEl.textContent = "Route plotted";
                } else {
                    map.setView(points[0], 8);
                    if (distanceEl) distanceEl.textContent = "Partial route — some addresses not located";
                }
            });
        ...
        // Start once Alpine has unhidden the container (loading flips after ~800ms).
        setTimeout(initShipmentMap, 900);
    });
</script>
@endsection
